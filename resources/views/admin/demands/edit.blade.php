...
<div class="card" style="background: white; border-radius: 12px; box-shadow: 0 2px 8px rgba(0,0,0,0.08); padding: 32px; max-width: 600px;">
    <form action="{{ route('admin.demands.update', $demand->id) }}" method="POST">
        @csrf
        @method('PUT')
        
        <div style="margin-bottom: 20px;">
            <label style="display: block; margin-bottom: 8px; font-weight: 500; color: #64748b;">Demand Name</label>
            <input type="text" name="name" value="{{ old('name', $demand->name) }}" required
        ...
        <div style="margin-bottom: 20px;">
            ...
            <textarea name="description" rows="3"
                      style="width: 100%; padding: 12px; border: 1px solid #e5e7eb; border-radius: 8px; font-size: 14px;">{{ old('description', $demand->description) }}</textarea>
